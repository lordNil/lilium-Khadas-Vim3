...
def yoloface_post_process(data):
    input0_data = data[0]
    input1_data = data[1]
    input2_data = data[2]
...
def draw(image, boxes, scores):
    for box, score in zip(boxes, scores):
        ...
        image = cv.rectangle(image, (top, left), (right, bottom), (255, 0, 0), 2)
        iamge = cv.putText(image, '{0} {1:.2f}'.format(CLASS, score),
                    (top, left - 6),
...
if __name__ == '__main__':
    ...
    while(1):
        cv_img = list()
        ret,frame = cap.read()
        frame = cv.flip( frame, flipCode=0 )  # rotate on x axis
        width = frame.shape[1]
        width_cut = width //2
